...
def compare_crossovers():
    ...
    test_operators = [
        TestOperatorWrapper(
            "K-1", VectorKPointsCrossover(probability=probability, k=1)
        ),
        TestOperatorWrapper(
            "K-5", VectorKPointsCrossover(probability=probability, k=5)
        ),
        TestOperatorWrapper(
            "K-10", VectorKPointsCrossover(probability=probability, k=10)
        ),
        TestOperatorWrapper("Uniform", VectorUniformCrossover(probability=probability)),
        TestOperatorWrapper(
            "Blend", FloatVectorBlendCrossover(probability=probability)
        ),
        TestOperatorWrapper("Mean", FloatVectorMeanCrossover(probability=probability)),
    ]
    for test_class in test_operators:
        crossover = test_class.test_operator
        result = float_vector_one_max_crossover_runner(
            repeats=repeats, threshold=threshold, crossover=crossover, length=length
        )
